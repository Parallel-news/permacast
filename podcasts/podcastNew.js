...
export async function handle(state, action) {
  ...
  // ERRORS List
  const ERROR_INVALID_CALLER = `the caller is not allowed to execute this function`;
  const ERROR_INVALID_PRIMITIVE_TYPE = `the given data is not a corrected primitive type per function`;
  const ERROR_INVALID_STRING_LENGTH = `the string is out of the allowed length ranges`;
  ...
  const ERROR_NEGATIVE_INTEGER = `negative integer was supplied when only positive Intare allowed`;
  const ERROR_EPISODE_INDEX_NOT_FOUND = `there is no episode with the given index`;
  const ERROR_PODCAST_INDEX_NOT_FOUND = `there is no podcast with the given index`;
